...
class InMemoryReactiveMessageStoreTest extends MessageStoreContractTest {
    ...
    @Override protected Message put(Message m) { return store.put(m).await().indefinitely(); }
    @Override protected Optional<Message> find(Long id) { return store.find(id).await().indefinitely(); }
    @Override protected List<Message> scan(MessageQuery q) { return store.scan(q).await().indefinitely(); }
    @Override protected Map<UUID, Long> countAllByChannel() { return store.countAllByChannel().await().indefinitely(); }
    @Override protected List<String> distinctSendersByChannel(UUID chId, MessageType ex) { return store.distinctSendersByChannel(chId, ex).await().indefinitely(); }
