...
| **A2A compatible** | ❌ | ❌ | ✅ via `ChannelGateway` — `AgentChannelBackend`, `HumanParticipatingChannelBackend`, `HumanObserverChannelBackend`; A2A protocol bridge in #135 | ✅ **Native** | 🔶 | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **ACP compatible** | ❌ | ❌ | 🔶 via `ChannelGateway` — same backend SPI pattern as A2A | 🔶 | ✅ **Native** | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
